...
def get_drf_stripe_user_model():
    if drf_stripe_settings.DJANGO_USER_MODEL:
        return django_apps.get_model(
            drf_stripe_settings.DJANGO_USER_MODEL, require_ready=False
        )
    else:
...
class StripeUser(models.Model):
    ...
    def subscribed_products(self):
        ...
        return {
            item.price.product
            for item in self.current_subscription_items.prefetch_related(
    ...
    def subscribed_features(self):
        ...
        product_list = Price.objects.filter(pk__in=price_list).values_list(
            "product", flat=True
        )
        return {
            item.feature
            for item in ProductFeature.objects.filter(
                product_id__in=product_list
            ).prefetch_related("feature")
        }

    class Meta:
        indexes = [models.Index(fields=["user", "customer_id"])]
...
class ProductFeature(models.Model):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, related_name="linked_features"
    )
    feature = models.ForeignKey(
        Feature, on_delete=models.CASCADE, related_name="linked_products"
    )
...
class Price(models.Model):
    ...
    )  # price in cents, corresponding to Stripe unit_amount
    ...
    class Meta:
        indexes = [models.Index(fields=["active", "freq"])]
...
class Subscription(models.Model):
    ...
    stripe_user = models.ForeignKey(
        StripeUser, on_delete=models.CASCADE, related_name="subscriptions"
    )
    ...
    class Meta:
        indexes = [models.Index(fields=["stripe_user", "status"])]
