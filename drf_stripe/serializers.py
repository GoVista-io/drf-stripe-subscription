...
class SubscriptionItemSerializer(serializers.ModelSerializer):
    ...
    cancel_at_period_end = serializers.BooleanField(
        source="subscription.cancel_at_period_end"
    )

    def get_feature_ids(self, obj):
        return [
            {
                "feature_id": link.feature.feature_id,
                "feature_desc": link.feature.description,
            }
            for link in obj.price.product.linked_features.all().prefetch_related(
                "feature"
            )
        ]
    ...
    def get_subscription_expires_at(self, obj):
        return (
            obj.subscription.period_end
            or obj.subscription.cancel_at
            or obj.subscription.trial_end
            or obj.subscription.ended_at
        )
...
class PriceSerializer(serializers.ModelSerializer):
    ...
    def get_feature_ids(self, obj):
        return [
            {
                "feature_id": prod_feature.feature.feature_id,
                "feature_desc": prod_feature.feature.description,
            }
            for prod_feature in obj.product.linked_features.all().prefetch_related(
                "feature"
            )
        ]
    ...
    def validate(self, attrs):
        ...
        try:
            checkout_session = stripe_api_create_checkout_session(
                ...
                trial_end=(
                    "auto" if stripe_user.subscription_items.count() == 0 else None
                ),
            )
